...
Shader "Toon-phone"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#endif
			CBUFFER_END
			UNITY_INSTANCING_BUFFER_START(Toonphone)
				UNITY_DEFINE_INSTANCED_PROP(float, _Float0)
			UNITY_INSTANCING_BUFFER_END(Toonphone)
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				Gradient gradient12_g2 = NewGradient( 0, 3, 2, float4( 0, 0, 0, 0.4800031 ), float4( 1, 1, 1, 0.5199969 ), float4( 1, 1, 1, 0.7617609 ), 0, 0, 0, 0, 0, float2( 1, 0 ), float2( 1, 1 ), 0, 0, 0, 0, 0, 0 );
				float3 ase_worldNormal = packedInput.ase_texcoord1.xyz;
				float3 normalizeResult15_g2 = normalize( ( -_DirectionalLightDatas[0].forward + V ) );
				float dotResult16_g2 = dot( ase_worldNormal , normalizeResult15_g2 );
				float _Float0_Instance = UNITY_ACCESS_INSTANCED_PROP(Toonphone,_Float0);
				
				surfaceDescription.Color = SampleGradient( gradient12_g2, pow( max( dotResult16_g2 , 0.0 ) , _Float0_Instance ) ).rgb;
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				float3 normalizeResult15_g2 = normalize( ( -_DirectionalLightDatas[0].forward + ase_worldViewDir ) );
				float dotResult16_g2 = dot( ase_worldNormal , normalizeResult15_g2 );
				float _Float0_Instance = UNITY_ACCESS_INSTANCED_PROP(Toonphone,_Float0);
...
WireConnection;8;13;9;0
WireConnection;0;0;8;0
ASEEND*/
//CHKSM=59C9F6E22389E55AFC9D73840E3D738B83893399