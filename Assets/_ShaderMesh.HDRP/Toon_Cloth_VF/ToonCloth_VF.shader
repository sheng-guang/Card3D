...
Shader "ToonCloth_VF"
{
	Properties
	{
		...
		[ASEBegin][HDR]_RimColor("Rim Color-------------------", Color) = (0,1,0.8758622,0)
		_RimPower("Rim Power", Range( 0 , 30)) = 0.8317261
		_RimOffset("Rim Offset", Range( -1 , 1)) = 0.24
		_color("color-------------", 2D) = "white" {}
		_envpowertimes("env power times", Range( 0 , 0.01)) = 0
		_maxcolor("max color", Range( 0 , 2)) = 1
		_mincolor("min color", Range( 0 , 2)) = 0.6
		_scale("scale", Float) = 1
		[ASEEnd]_highlightpower("high light power", Range( 0 , 15)) = 7.62
	...
	SubShader
	{
		...
		Pass
		{
			...
			struct VertexOutput
			{
				...
				float4 ase_lightmapUVs : TEXCOORD2;
			...
			CBUFFER_END
			sampler2D _color;
			UNITY_INSTANCING_BUFFER_START(ToonCloth_VF)
				UNITY_DEFINE_INSTANCED_PROP(float, _highlightpower)
			UNITY_INSTANCING_BUFFER_END(ToonCloth_VF)
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float dotResult4_g34 = dot( ase_worldNormal , V );
				Gradient gradient12_g35 = NewGradient( 0, 3, 2, float4( 0, 0, 0, 0.4800031 ), float4( 1, 1, 1, 0.5199969 ), float4( 1, 1, 1, 0.7617609 ), 0, 0, 0, 0, 0, float2( 1, 0 ), float2( 1, 1 ), 0, 0, 0, 0, 0, 0 );
				float dotResult4_g35 = dot( reflect( _DirectionalLightDatas[0].forward , ase_worldNormal ) , V );
				float _highlightpower_Instance = UNITY_ACCESS_INSTANCED_PROP(ToonCloth_VF,_highlightpower);
				Gradient gradient13_g33 = NewGradient( 0, 3, 2, float4( 0, 0, 0, 0.4800031 ), float4( 1, 1, 1, 0.5199969 ), float4( 1, 1, 1, 0.7617609 ), 0, 0, 0, 0, 0, float2( 1, 0 ), float2( 1, 1 ), 0, 0, 0, 0, 0, 0 );
				float dotResult9_g33 = dot( ase_worldNormal , -_DirectionalLightDatas[0].forward );
				float4 temp_cast_1 = (_mincolor).xxxx;
				float4 temp_cast_2 = (_maxcolor).xxxx;
				float3 ase_worldPos = GetAbsolutePositionWS( positionRWS );
				float3 bakedGI26 = SampleBakedGI( ase_worldPos, ase_worldNormal, packedInput.ase_lightmapUVs.xy, packedInput.ase_lightmapUVs.zw );
				float4 screenPos = packedInput.ase_texcoord3;
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float4 appendResult120 = (float4(ase_screenPosNorm.x , ( ase_screenPosNorm.y * ( _ScreenParams.y / _ScreenParams.x ) ) , 0.0 , 0.0));
				float4 tex2DNode36 = tex2D( _color, (appendResult120*_scale + 0.0).xy );
				
				surfaceDescription.Color = ( ( ( 1.0 * saturate( pow( ( 1.0 - saturate( ( dotResult4_g34 + _RimOffset ) ) ) , _RimPower ) ) ) * ( _RimColor * float4( saturate( _DirectionalLightDatas[0].color.rgb ) , 0.0 ) ) ) + ( ( SampleGradient( gradient12_g35, pow( max( dotResult4_g35 , 0.0 ) , _highlightpower_Instance ) ) + (temp_cast_1 + (SampleGradient( gradient13_g33, saturate( (dotResult9_g33*0.5 + 0.5) ) ) - float4( 0,0,0,0 )) * (temp_cast_2 - temp_cast_1) / (float4( 1,1,1,1 ) - float4( 0,0,0,0 ))) + float4( ( bakedGI26 * _envpowertimes ) , 0.0 ) ) * tex2DNode36 ) ).rgb;
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				float dotResult4_g34 = dot( ase_worldNormal , ase_worldViewDir );
				Gradient gradient12_g35 = NewGradient( 0, 3, 2, float4( 0, 0, 0, 0.4800031 ), float4( 1, 1, 1, 0.5199969 ), float4( 1, 1, 1, 0.7617609 ), 0, 0, 0, 0, 0, float2( 1, 0 ), float2( 1, 1 ), 0, 0, 0, 0, 0, 0 );
				float dotResult4_g35 = dot( reflect( _DirectionalLightDatas[0].forward , ase_worldNormal ) , ase_worldViewDir );
				float _highlightpower_Instance = UNITY_ACCESS_INSTANCED_PROP(ToonCloth_VF,_highlightpower);
				Gradient gradient13_g33 = NewGradient( 0, 3, 2, float4( 0, 0, 0, 0.4800031 ), float4( 1, 1, 1, 0.5199969 ), float4( 1, 1, 1, 0.7617609 ), 0, 0, 0, 0, 0, float2( 1, 0 ), float2( 1, 1 ), 0, 0, 0, 0, 0, 0 );
				float dotResult9_g33 = dot( ase_worldNormal , -_DirectionalLightDatas[0].forward );
				float4 temp_cast_1 = (_mincolor).xxxx;
...
/*ASEBEGIN
Version=18933
0;228;1385;683;1603.432;1684.038;1.357143;True;False
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;100;254.4256,-800.3686;Inherit;True;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;96;-1201.427,-1469.767;Inherit;False;InstancedProperty;_highlightpower;high light power;8;0;Create;True;0;0;0;False;0;False;7.62;5.4;0;15;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;104;-800.3582,-1429.326;Inherit;False;ToonHightLight;-1;;35;1fe4f530f70aab84086cf91b33178994;0;1;13;FLOAT;7.62;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;118;-896.0562,-559.1292;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;73;-55.75546,-1397.531;Float;False;Property;_RimPower;Rim Power;1;0;Create;True;0;0;0;False;0;False;0.8317261;30;0;30;0;1;FLOAT;0
Node;AmplifyShaderEditor.ScreenParams;116;-1262.056,-500.1292;Inherit;False;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ScreenPosInputsNode;108;-1196.21,-711.1036;Float;False;0;False;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleAddOpNode;115;553.4847,-887.4542;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;75;-56.19052,-1325.543;Float;False;Property;_RimOffset;Rim Offset;2;0;Create;True;0;0;0;False;0;False;0.24;-0.26;-1;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.ScaleAndOffsetNode;112;-600.249,-580.136;Inherit;True;3;0;FLOAT4;0,0,0,0;False;1;FLOAT;1;False;2;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.SimpleDivideOpNode;117;-1026.056,-510.1292;Inherit;False;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;34;-49.93349,-1239.733;Float;False;Property;_RimColor;Rim Color-------------------;0;1;[HDR];Create;True;0;0;0;False;0;False;0,1,0.8758622,0;0.5754717,0.4270525,0.1764418,0.8078431;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.FunctionNode;85;245.7667,-1069.703;Inherit;True;rim;-1;;34;50b81ff80f24ef24493b47e052379a4c;0;4;21;FLOAT;1;False;19;FLOAT;0.5;False;18;FLOAT;0;False;20;COLOR;0,1,0.8758622,0;False;1;COLOR;0
Node;AmplifyShaderEditor.IndirectDiffuseLighting;26;-839.1607,-955.1906;Inherit;True;Tangent;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.DynamicAppendNode;120;-752.0562,-614.1292;Inherit;False;FLOAT4;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;106;-448.2538,-1022.748;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleAddOpNode;103;-182.1081,-1019.796;Inherit;True;3;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT3;0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.FunctionNode;121;-849.959,-1549.944;Inherit;False;ToonHightLight(b-phone);-1;;32;9c0b70fe788c96a4aa8c0ffcabb653d3;0;1;13;FLOAT;5;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;113;-877.249,-422.136;Inherit;False;Property;_scale;scale;7;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;110;-1352.683,-1299.29;Inherit;False;Property;_maxcolor;max color;5;0;Create;True;0;0;0;False;0;False;1;1.085;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;107;-830.754,-735.8549;Inherit;False;Property;_envpowertimes;env power times;4;0;Create;True;0;0;0;False;0;False;0;0.00256;0;0.01;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;109;-1322.683,-1379.29;Inherit;False;Property;_mincolor;min color;6;0;Create;True;0;0;0;False;0;False;0.6;0;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.GammaToLinearNode;114;48.05396,-556.8064;Inherit;True;0;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SamplerNode;36;-271.6801,-605.5104;Inherit;True;Property;_color;color-------------;3;0;Create;True;0;0;0;False;0;False;-1;cf6458571683a21438f14637c025d623;e85ae53546672104780f7a98a2118211;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;1;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.FunctionNode;102;-995.962,-1343.259;Inherit;False;Toon diffuse;-1;;33;f8624219d58f6e645bc9569650429b82;0;2;14;FLOAT;0.6;False;15;FLOAT;1;False;1;COLOR;0
...
WireConnection;104;13;96;0
WireConnection;118;0;108;2
...
WireConnection;85;19;73;0
WireConnection;85;18;75;0
WireConnection;85;20;34;0
WireConnection;120;0;108;1
...
WireConnection;121;13;96;0
WireConnection;114;0;36;0
WireConnection;36;1;112;0
WireConnection;102;14;109;0
WireConnection;102;15;110;0
WireConnection;0;0;115;0
ASEEND*/
//CHKSM=45319B83A523E35420F2B8AC234F0A202BF96C93